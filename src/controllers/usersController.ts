...
export const editProfile = async (req: RequestWithToken, res: Response) => {
  try {
    const { username, firstname, lastname, phone, rolename, password } = req.body;
    const userId = req.params.id;

    const currentProfileQuery = `
      SELECT username, firstname, lastname, phone, rolename, password
      FROM master
      WHERE id = $1
    `;
    ...
    let updatedPassword = currentProfile.password;
    ...
    const updatedProfile = {
      ...
      rolename: rolename || currentProfile.rolename,
      password: updatedPassword, 
    };

    const updateProfileQuery = `
      UPDATE master
      SET username = $1, firstname = $2, lastname = $3, phone = $4, rolename = $5, password = $6
      WHERE id = $7
      RETURNING *
    `;
...
export const getCaregiven = async (req: RequestWithToken, res: Response) => {
  try {
    ...
    const caregivenTotalResult = await pool.query(`SELECT id, ssd, firstname, lastname, sex, age, phone, caregiver, operating_area FROM caregiven`)
    ...
    return res.status(200).json({
      totalCaregiven,
      allCaregiven,
      caregivenTotalResult:caregivenTotalResult.rows,
...
export const getUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const allUserTotal = await pool.query('SELECT id, ssd, firstname, lastname, sex, age, phone FROM users')
    ...
    return res.status(200).json({
      ...
      totalAllDisabled,
      totalAllNormal,
      allUserTotal: allUserTotal.rows,
  ...
  try {
    ...
    const HelthData = await pool.query(
      `SELECT id, type_of_disability, cause_of_disability, financial_support, medical_treatment_rights,
                    assistive_equipment, ability_to_carry_out_daily_activities,career_and_work, education_and_training, need_for_assistance,
...
export const getDataAdlFromId = async (req: Request, res: Response) => {
  ...
  try {
    const query = `
        SELECT time_stamp, adl_target,adl_point
        FROM adl_form
        WHERE users_id = $1
        ORDER BY time_stamp DESC
      `;
