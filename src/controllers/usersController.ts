...
export const createMaster = async (req: RequestWithToken, res: Response) => {
    try {
        ...
        const checkUsername = await pool.query('SELECT * FROM master WHERE username = $1', [req.body.username]);
        if (checkUsername.rows.length === 0) {
            const result = await pool.query('INSERT INTO master (username,password,firstname,lastname,rolename,role,phone,statusmaster) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *',
                [req.body.username, hashPassword, req.body.firstname, req.body.lastname, req.body.rolename,role, req.body.phone,'READY']);
...
export const updateStatusMaster = async (req: RequestWithToken, res: Response) => {
    try {
        const checkStatus = await pool.query(`SELECT statusmaster FROM master WHERE id = $1`,[req.params.id])
        const checkid = await pool.query(`SELECT id FROM master WHERE id = $1`,[req.params.id]) 
        ...
        if(checkStatus.rows[0].statusmaster === 'READY'){
            result = await pool.query(`UPDATE master SET statusmaster = $1 WHERE id = $2 RETURNING *`,['NOTREADY',req.params.id])
        }
        if(checkStatus.rows[0].statusmaster === 'NOTREADY'){
            result = await pool.query(`UPDATE master SET statusmaster = $1 WHERE id = $2 RETURNING *`,['READY',req.params.id])
        }
...
export const allMaster = async (req: RequestWithToken, res: Response) => {
    try {
        const allMaster = await pool.query('SELECT id,firstname,lastname,rolename FROM master')
...
export const getProfile = async (req: RequestWithToken, res: Response) => {
    try {
        const Master = await pool.query('SELECT id,username,firstname,lastname,phone,rolename FROM master WHERE id = $1',[req.params.id])
...
export const profileMasterByid = async (req: RequestWithToken, res: Response) => {
    try {
        const Master = await pool.query('SELECT id,username,firstname,lastname,role,phone FROM master WHERE id = $1',[req.token?.id])
...
export const getCaregivenFromuser = async (req: RequestWithToken, res: Response) => {
    try {
        const result = await pool.query(
            `SELECT 
...
export const getCaregivenFromId = async (req: RequestWithToken, res: Response) => {
    try {
        const result = await pool.query(
            `SELECT 
...
export const getUser = async (req: RequestWithToken, res: Response) => {
    ...
    try {
        ...
        const allDisabled = await pool.query(`
            SELECT id, ssd, firstname, lastname, sex, age, phone 
            FROM users 
            WHERE type = 'มีความพิการ' 
            LIMIT $1 OFFSET $2
        `, [limit, offset]);
  
  
        const allNormal = await pool.query(`
            SELECT id, ssd, firstname, lastname, sex, age, phone 
            FROM users 
            WHERE type = 'สุขภาวะปกติ' 
            LIMIT $1 OFFSET $2
        `, [limit, offset]);
        ...
        return res.status(200).json({
            ...
            allDisabledCount: allDisabled.rowCount,
            allDisabled: allDisabled.rows,
            allNormalCount: allNormal.rowCount,
            allNormal: allNormal.rows,
        });
  ...
  export const getusersBySSD = async (req: RequestWithToken, res: Response) => {
    try {
        const resultData = await pool.query('SELECT id, ssd, firstname, lastname, sex, age, phone FROM users WHERE ssd = $1',[req.params.id])
  ...
  export const getcaregivenBySSD = async (req: RequestWithToken, res: Response) => {
    try {
        const result = await pool.query(`SELECT id,ssd,firstname,lastname,sex,age,phone,caregiver,operating_area FROM caregiven WHERE ssd = $1`,[req.params.id])
        if(result.rowCount === 0){
            res.status(404).send('Not found')
        }else{
            res.status(200).send(result.rows)
  ...
  export const dataCaregiven = async (req: RequestWithToken, res: Response) => {
    try {
      ...
      const result = await pool.query(`SELECT id,ssd,firstname,lastname,sex,age,birthday_date,num_of_house,group_of_house,tambon,amphoe,province,postcode,phone,line_id,education_level,email,career,caregiver,operating_area FROM caregiven
